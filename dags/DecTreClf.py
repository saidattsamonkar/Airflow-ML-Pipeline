...
def func_DTC(**kwargs):
    ...
    y_pred = dtc_model.transform(test_df)

    y_pred = y_pred.toPandas()
    y_pred.to_csv('dtc_y_pred.csv')
    with open('dtc_y_pred.csv', 'rb') as f:
        s3.put_object(Bucket='airflow-customer-churn-mlops', Key='dtc_y_pred.csv', Body=f)

    #kwargs['ti'].xcom_push(key='dtc_y_pred', value='dtc_y_pred.csv')

    dtc_model.write().overwrite().save('dtc_model.model')
    shutil.make_archive('dtc_model', 'gztar', root_dir='dtc_model.model')
    with open('dtc_model.tar.gz', 'rb') as f:
        s3.put_object(Bucket='airflow-customer-churn-mlops', Key='dtc_model.tar.gz', Body=f)
